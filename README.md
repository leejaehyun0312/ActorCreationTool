...
![EventButton Method Binding](./Docs/eventbutton-method-binding.png)

### [Editor Preview](./EditoPreview)

생성된 모델을 별도의 Scene으로 이동하지 않고 ACT 내부에서 확인하고 편집하기 위한 Preview UI입니다.

![Editor Preview](./Docs/preview-overview.png)
...
```text
ActorCreationTool/
├─ Pipeline/       비동기 실행 규약과 순차 실행 관리
├─ Blueprint/      페이지 구성과 Wizard EditorWindow
├─ EventButton/    UI Builder 메서드 바인딩
├─ EditoPreview/   SceneViewElement와 Preview Hierarchy
├─ Docs/           기능 이미지와 GIF
└─ README.md
```
